...
const registerCountrySelection = async (req, res) => {
  const { name, email, countries, universities } = req.body;
  ...
  // Join arrays to comma-separated strings for DB
  ...
  db.query(sql, [name, email, countryList, universityList], async (err, result) => {
    if (err) {
      console.error("Database error:", err);
      return res.status(500).json({ message: "Database error" });
    }
