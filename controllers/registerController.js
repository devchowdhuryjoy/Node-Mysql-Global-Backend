...
//     return res.status(400).json({ message: 'Preferred destination is required' });
//   }
//   if (!testStatus) {
...
//   // Phone validation (+8801 or 01 start)
...
const registerUser = async (req, res) => {
  ...
  if (!nearestOffice) return res.status(400).json({ message: 'Nearest office is required' });
  if (!preferredDestination) return res.status(400).json({ message: 'Preferred destination is required' });
  if (!testStatus) return res.status(400).json({ message: 'English test status is required' });
  if (!fundingPlan) return res.status(400).json({ message: 'Funding plan is required' });
...
Global Routeway Consult Team
    ...
    try {
      ...
      res.status(200).json({ message: 'Registration successful but email failed to send' });
    }
  });
};
